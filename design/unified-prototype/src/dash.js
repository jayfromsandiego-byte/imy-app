...
function boot(ctx){
  ...
  var rows=mems.map(function(m){
    ...
    return '<div class="mem-row'+(act&&m.id===act.id?' now':'')+'">'+
      '<div class="mr-main"><b>'+esc(name)+'</b><span class="mr-url">imissyoumemorial.com/'+esc(m.slug)+'</span>'+(m.owner?'<span class="mr-owner">ownership passing to '+esc(m.owner)+'</span>':'')+'</div>'+
  ...
  /* account */
  cards.push('<div class="s-card full stagger" id="acctCard"><p class="sentence">Signed in as <b>'+esc(acct.name||'')+'</b> · '+esc(acct.email||'')+(acct.method==='google'?' · Google':'')+'.</p>'+
    '<div class="card-foot"><a href="#" class="btn quiet" data-act="signout">Sign out</a>'+
    (act?'<a href="#" class="btn quiet" data-act="ownership">Pass ownership of '+esc(first||'the')+'’s page</a>':'')+
    '<a href="#" class="btn quiet danger" data-act="delacct">Delete this account</a></div></div>');
  ...
  /* ── two-door verifications · anchored on the original flows ── */
  function modal(o){
    var veil=document.createElement('div');veil.className='imy-veil';
    veil.innerHTML='<div class="imy-modal" role="dialog" aria-modal="true">'+
      '<h4>'+o.title+'</h4>'+(o.sub?'<p>'+o.sub+'</p>':'')+
      (o.gate?'<input type="'+(o.gate.type||'text')+'" id="imGate" autocomplete="off" spellcheck="false" placeholder="'+esc(o.gate.placeholder||'')+'"/>':'')+
      '<div class="rowbtns"><button type="button" class="btn quiet" id="imNo">'+(o.quiet||'Cancel')+'</button>'+
      '<button type="button" class="btn '+(o.danger?'dangerfill':'primary')+'" id="imYes"'+(o.gate?' disabled':'')+'>'+o.confirm+'</button></div></div>';
    document.body.appendChild(veil);
    var yes=veil.querySelector('#imYes'), gate=veil.querySelector('#imGate');
    function close(){try{document.body.removeChild(veil)}catch(e){}}
    veil.addEventListener('click',function(e){if(e.target===veil)close()});
    veil.querySelector('#imNo').onclick=close;
    document.addEventListener('keydown',function esc2(e){if(e.key==='Escape'){close();document.removeEventListener('keydown',esc2)}});
    if(gate){
      gate.addEventListener('input',function(){
        var v=gate.value.trim();
        var ok=o.gate.match?(v.toLowerCase()===String(o.gate.match).toLowerCase()):/.+@.+\..+/.test(v);
        yes.disabled=!ok;
      });
      setTimeout(function(){gate.focus()},60);
    }
    yes.onclick=function(){ if(yes.disabled)return; var v=gate?gate.value.trim():''; close(); o.onConfirm(v); };
  }
  function deleteFlow(){
    modal({
      title:'Delete this account?',
      sub:'The pages it keeps, the memories, the photographs — everything goes with it.',
      quiet:'No, keep everything', confirm:'Yes, continue', danger:true,
      onConfirm:function(){
        modal({
          title:'This is permanent.',
          sub:'To confirm, type the account email exactly: <b>'+esc(acct.email||'')+'</b>',
          gate:{placeholder:acct.email||'',match:acct.email||'',type:'email'},
          quiet:'Cancel', confirm:'Delete the account', danger:true,
          onConfirm:function(){ IMY.send('deleteAccount',{}); }
        });
      }
    });
  }
  function ownershipFlow(){
    if(!act)return;
    var pageName=(first?first+'’s':'the')+' page';
    modal({
      title:'Pass ownership of '+esc(pageName)+'?',
      sub:'They become the keeper — the page, its memories, and its keys move to them.',
      gate:{placeholder:'their email',match:null,type:'email'},
      quiet:'Cancel', confirm:'Continue',
      onConfirm:function(email1){
        modal({
          title:'Are you sure you want to make them the owner?',
          sub:'Type their email once more to confirm: <b>'+esc(email1)+'</b>',
          gate:{placeholder:email1,match:email1,type:'email'},
          quiet:'Cancel', confirm:'Make them the owner',
          onConfirm:function(){ IMY.send('transferOwner',{m:act.id,email:email1}); }
        });
      }
    });
  }
  ...
  document.addEventListener('click',function(e){
    ...
    if(v==='delacct'){deleteFlow();return}
    if(v==='ownership'){ownershipFlow();return}
