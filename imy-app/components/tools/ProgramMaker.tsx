...
.kb-form label{font-family:'Sometype Mono',ui-monospace,monospace;font-size:11px;letter-spacing:.12em;text-transform:uppercase;color:rgba(44,37,32,.6);display:block;margin-bottom:5px;}
...
.kb-privacy{font-size:13px;color:rgba(44,37,32,.6);border-left:3px solid #C9A572;padding-left:12px;}
...
.kb-cta a{color:#A87C5F;text-decoration:none;border-bottom:1px solid rgba(168,124,95,.4);}
...
.kb-cover-dates{font-family:'Sometype Mono',ui-monospace,monospace;font-size:9px;letter-spacing:.1em;color:#A87C5F;margin:6px 0 0;}
.kb-cover-line{font-size:9.5px;color:rgba(44,37,32,.65);margin:8px 0 0;}
.kb-lim{font-family:'Sometype Mono',ui-monospace,monospace;font-size:8px;letter-spacing:.16em;text-transform:uppercase;color:#A87C5F;margin:0 0 4%;}
.kb-oi{margin:0 0 5%;width:100%;}
.kb-oi b{display:block;font-size:10.5px;font-weight:600;}
.kb-oi span{font-size:9px;color:rgba(44,37,32,.65);}
...
`,
      ...
      <div className="kb-form">
        <div>
          ...
          <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Eleanor Margaret Hayes" />
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          <div>
            <label>Sunrise</label>
            <input type="text" value={sunrise} onChange={(e) => setSunrise(e.target.value)} placeholder="June 4, 1948" />
          </div>
          <div>
            <label>Sunset</label>
            <input type="text" value={sunset} onChange={(e) => setSunset(e.target.value)} placeholder="November 18, 2024" />
          </div>
        </div>
        <div>
          <label>Service line</label>
          <input type="text" value={serviceLine} onChange={(e) => setServiceLine(e.target.value)} placeholder="Saturday, November 23 · 11 am · St. Mary's Chapel" />
        </div>
        <div>
        ...
        <div className="kb-order">
          ...
          {items.map((it, i) => (
            <div className="kb-order-row" key={i}>
              <input type="text" value={it.label} onChange={(e) => setItem(i, { label: e.target.value })} />
              <input type="text" value={it.detail} placeholder="detail (optional)" onChange={(e) => setItem(i, { detail: e.target.value })} />
            </div>
          ))}
          <button
            type="button"
        ...
        {ready && !unlocked && (
          <form className="kb-gate" onSubmit={unlock}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" style={{ flex: 1 }} />
