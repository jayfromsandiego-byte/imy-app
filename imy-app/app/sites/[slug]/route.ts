...
// The living example every "see an example" link leads to.
const EXAMPLE_SLUG = "eleanor";

export async function GET(req: NextRequest, { params }: { params: { slug: string } }) {
  const slug = params.slug === "example" ? EXAMPLE_SLUG : params.slug;
  ...
  // ── the unified design, behind a flag (default OFF for families' pages).
  // Three doors: `?design=unified` previews the unified page for any slug (the
  // owner's production preview, never cached), env TRIBUTE_UNIFIED=1 flips the
  // default for the full cutover, and the example page leads the way.
  //
  // Why the example is always unified (owner walkthrough, Aug 31): the site's
  // example links must land on the NEW design, and /sites/eleanor is the one
  // indexable page that shows Google what a tribute actually looks like — it
  // sits in the sitemap with a self-canonical. Redirecting it into the app
  // bundle would have pointed every example link, and the sitemap entry, at a
  // noindex,nofollow document: the example would have quietly left the index.
  // So the old address becomes the new example instead. Nothing orphans, no
  // redirect is needed, the URL keeps its canonical, and the design is new.
  // `?design=wreath` keeps the old render reachable for comparison.
  const designParam = req.nextUrl.searchParams.get("design");
  const useUnified =
    designParam === "unified" ||
    process.env.TRIBUTE_UNIFIED === "1" ||
    (slug === EXAMPLE_SLUG && designParam !== "wreath");
